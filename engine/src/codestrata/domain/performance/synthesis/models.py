"""Performance synthesis domain models (Phase 4.9.5).

Themes, conclusions, and recommendations derived from the assessment inventory.
No scores, grades, performance indices, modernization guidance, or AI narrative.
"""
...
class PerformanceTheme(BaseModel):
    """Inventory-derived Performance theme (no synthetic score)."""
    ...
    title: str
    description: str
    scope: PerformanceThemeScope = PerformanceThemeScope.REPOSITORY
...
class PerformanceConclusion(BaseModel):
    """Deterministic Performance conclusion with bounded template text."""
    ...
    assessment_dimensions: tuple[str, ...] = ("performance",)
    business_impact: str = "unknown"
    confidence: str = "high"
    provenance: str = "performance_synthesis"
...
class PerformanceSynthesisResult(BaseModel):
    """Complete synthesis payload attached to the assessment section."""

    model_config = ConfigDict(frozen=True, extra="forbid")

    status: PerformanceSynthesisStatus = PerformanceSynthesisStatus.SUCCEEDED
