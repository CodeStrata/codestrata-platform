"""Security assessment identifiers and constants (Phase 4.5.1)."""
...
SECTION_ID = "assessment.security"
SCHEMA_NAME = "security-assessment"
SECTION_SCHEMA_VERSION = "1.3.0"
SECURITY_ASSESSMENT_FILENAME = "security-assessment.json"
ARTIFACT_SCHEMA_ID = "codestrata.security_assessment"

MAX_SECURITY_HOTSPOTS = 20
MAX_HOTSPOT_FINDING_REFS = 25
MAX_TRACEABILITY_ENTRIES = 12


def build_hotspot_id(*, path: str, source_role: str = "aggregate") -> str:
    payload = f"{path.strip().lower()}|{source_role.strip().lower()}"
    digest = hashlib.sha256(payload.encode("utf-8")).hexdigest()[:12]
    return f"sec-hotspot:{digest}"
