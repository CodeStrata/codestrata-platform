...
from codestrata.config.settings import CodestrataSettings, DependencyAssessmentSectionSettings
...
def dependency_assessment_section_enabled(settings: CodestrataSettings | None) -> bool:
    if settings is None:
        return False
    return bool(settings.assessment.sections.dependency.enabled)


def dependency_pack_enabled(settings: CodestrataSettings | None) -> bool:
    if settings is None:
        return False
