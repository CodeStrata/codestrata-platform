"""Opt-in Test Intelligence execution for assessment (Phase 4.6.3).

Runs only when ``[rules] enabled = true`` and ``[rules.testing] enabled = true``.
Repository-testing evidence must be supplied in-memory. This module never
...
class TestingPackExecutionResult(BaseModel):
    """Testing pack evaluation plus evidence fingerprints."""

    model_config = ConfigDict(frozen=True, extra="forbid")

    evaluation: RuleEvaluationResult
    repository_testing_evidence: AggregatedRepositoryTestingEvidence | None = None
    ...
    rule_execution_facts: tuple[TestingRuleExecutionFact, ...] = ()
    rules_failed: int = Field(default=0, ge=0)


def evaluate_testing_pack_detailed(
    *,
    ...
    return evaluate_testing_pack_for_context_detailed(
...
def evaluate_testing_pack_for_context_detailed(
    *,
    legacy_context: RuleContext,
    settings: CodestrataSettings,
    repository_testing_evidence: AggregatedRepositoryTestingEvidence | None = None,
) -> TestingPackExecutionResult:
    """Evaluate testing.core using only the supplied evidence bundle."""
    ...
    frameworks_declared = 0

    if evidence is None:
        diagnostics.append("repository_testing_evidence_unavailable")
    else:
        evidence_pipeline = "repository_testing"
        evidence_fingerprint = evidence.evidence_fingerprint or _fingerprint_evidence(
            evidence
        )
        candidate_files_discovered = evidence.coverage.candidate_test_files_discovered
        frameworks_declared = evidence.coverage.frameworks_declared
        ...
        update={
            ...
            "provenance": {
                **shared_context.provenance,
                "testing_pack": TestingRulePack.pack_id,
                "testing_pack_version": TestingRulePack.pack_version,
                "repository_testing_evidence_fingerprint": evidence_fingerprint,
            },
            "configuration_facts": {
                **shared_context.configuration_facts,
                "testing_enabled": "true",
                "repository_testing_evidence_present": str(
    ...
    category_by_rule = {
        str(record.rule_id): record.category or RuleCategory.TESTING
    ...
    return TestingPackExecutionResult(
        evaluation=evaluation,
        repository_testing_evidence=evidence,
        evidence_pipeline=evidence_pipeline,
        evidence_fingerprint=evidence_fingerprint,
        candidate_files_discovered=candidate_files_discovered,
        frameworks_declared=frameworks_declared,
...
def testing_rules_planned_count() -> int:
    return len(HYGIENE_RULE_IDS)


def _fingerprint_evidence(evidence: AggregatedRepositoryTestingEvidence) -> str:
    if evidence.evidence_fingerprint:
        return evidence.evidence_fingerprint
    payload = (
        f"{evidence.repository_id}|{evidence.status.value}|"
        f"{len(evidence.file_candidates)}|{len(evidence.framework_facts)}|"
        f"{len(evidence.marker_facts)}"
    )
