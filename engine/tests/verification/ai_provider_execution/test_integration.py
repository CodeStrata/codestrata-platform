"""SV.11.4 full integration: run the verification suite end-to-end and inspect the report."""
...
from verification.ai_provider_execution.runner import (
    engine_root_from_package,
    run_ai_provider_execution_verification,
)
...
def test_run_verification_end_to_end(tmp_path: Path) -> None:
    ...
    report = run_ai_provider_execution_verification(engine_root=ENGINE_ROOT, output_dir=output_dir)
...
def test_scenario_ids_span_a_through_z() -> None:
    report = run_ai_provider_execution_verification(
...
def test_report_is_deterministic_across_two_runs(tmp_path: Path) -> None:
    first = run_ai_provider_execution_verification(
        engine_root=ENGINE_ROOT, output_dir=tmp_path / "r1"
    )
    second = run_ai_provider_execution_verification(
...
def test_report_excludes_forbidden_fragments(tmp_path: Path) -> None:
    from verification.ai_provider_execution.reporting import report_contains_forbidden_leak

    output_dir = tmp_path / "reports"
    run_ai_provider_execution_verification(engine_root=ENGINE_ROOT, output_dir=output_dir)
...
def test_report_matches_default_output_location() -> None:
    """The default output dir must be reports/verification/sv11-4 (not sv11-3)."""

    from verification.ai_provider_execution.contract import OUTPUT_RELATIVE

    assert OUTPUT_RELATIVE == "reports/verification/sv11-4"
