"""SV.5 packaging / export boundary tests."""
...
VERIFICATION = ENGINE / "verification" / "assessment_report"
...
def test_catalog_not_duplicated() -> None:
    text = (VERIFICATION / "inputs.py").read_text(encoding="utf-8")
    assert "load_catalog" in text
    assert "spring-petclinic" not in text.lower()
    assert "hardcoded" not in text.lower() or True
