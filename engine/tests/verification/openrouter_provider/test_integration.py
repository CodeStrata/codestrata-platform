...
from verification.openrouter_provider.reporting import report_directory
...
def report(tmp_path_factory: pytest.TempPathFactory):
    out = tmp_path_factory.mktemp("sv11-9")
...
def test_report_directory() -> None:
    assert report_directory(engine_root()).as_posix().endswith("reports/verification/sv11-9")
