...
1. **Three framework-leakage findings** sourced from
   `.codestrata/workspace/spring-petclinic` (workspace contamination).
...
assessing repository `path = "."` while `.codestrata/workspace/spring-petclinic`
...
- `LocalRepositoryScanner.DEFAULT_EXCLUDED_DIRECTORIES` does **not** include
  `.codestrata`.
- A local scan of `.` currently includes **~1173 paths under `.codestrata/`** and
...
| ---------- | ---- | ----- | ------------------------ | ----- |
| `...dependency-cycle:5beba100...` | dependency-cycle | `codestrata.application` ↔ `codestrata.infrastructure` | Yes | Stable since 4.2.1a |
| `...invalid-dependency-direction:ed578f0e...` | invalid-direction | application → infrastructure | Yes | Stable |
| `...excessive-cross-module-coupling:e304410e...` | coupling | `codestrata.application` (`out:10`) | Yes | Stable Option A |
...
| coupling `e304410e` (`codestrata.application`) | **Accept** | High | Distinct from boundary | Yes | Yes | Keep |
...
| framework-leakage `186c0b1b` | **False positive** (for this repo assess) | High as JPA-in-model observation; **low** as CodeStrata concern | Overlaps sibling leakages | Medium OK for real domain leakage | High too strong for contaminated input | Exclude `.codestrata` from scan/view |
...
not ingest `.codestrata/workspace` clones.
...
| broad-dependency `e05899c6` (`codestrata.application`) | coupling `e304410e` | **Accept** | Medium-High | None | Keep |
...
| framework-erosion ×3 | one leakage each | **Duplicate** / fragmentation | Low as three cards | High overlap | Cluster by package/unit; or disappear after `.codestrata` exclusion |
...
| Workspace-relative `.codestrata/workspace/...` paths present | Fail product intent (contamination visible) |
...
1. **`.codestrata` workspace contamination** in repository scan / architecture file
...
- Dual rule engines and codestrata/CodeStrata naming debt remain.

### Required corrections (narrow)

1. **Scanner / architecture path exclusion:** default-exclude `.codestrata` (and
...
| ------ | ------------------------------ | ------------------- |
| Workspace clone treated as product source | `LocalRepositoryScanner` (+ architecture ignore markers) | Exclude `.codestrata` by default |
...
- Confirmed scanner includes `.codestrata` paths
