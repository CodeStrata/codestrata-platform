...
| `dependency.mutable-version` | MEDIUM | HIGH | Bounded mutable syntax: `*-SNAPSHOT`, `latest`, `latest.release`, `RELEASE`, `+`, `N.+`, Composer `dev-*` |
| `dependency.unbounded-requirement` | MEDIUM | HIGH | Python registry-style declaration with no version specifier, or Composer declaration with empty/`*` constraint |
...
Supported languages for the Dependency Intelligence pack: `java`, `python`,
`php` (Phase 5.17.1), `csharp` (Phase 5.18). Composer ordinary ranges (`^`, `~`)
are **not** mutable. NuGet floating `*` constraints are unbounded findings.
