...
# Test Intelligence Domain Foundation
...
| Schema version | `1.2.0` (foundation `1.0.0` → inventory `1.1.0` → synthesis `1.2.0`) |
...
lifecycle fixtures. Hygiene findings use `assemble(...)`. Inventories and
deterministic synthesis fields are additive on schema **1.2.0**; scoring /
release-readiness claim fields remain forbidden.
