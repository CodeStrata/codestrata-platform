"""CLI entry: ``python -m verification.ai_provider_platform_completion``."""
...
from verification.ai_provider_platform_completion.reporting import (
    report_directory,
    verdict_is_allowed,
)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="SV.11.13 AI Provider Platform Completion Verification"
    )
    ...
    print(f"SV.11.13 AI Provider Platform Completion Verification: {report.verdict}")
    print(
        f"checks={report.check_counts} scenarios={len(report.negative_scenarios)} "
