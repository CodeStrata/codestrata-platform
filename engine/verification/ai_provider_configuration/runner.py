"""SV.11.3 Standardized Provider and Model Configuration verification — orchestration runner."""

from __future__ import annotations

from pathlib import Path

from verification.ai_provider_configuration import (
...
def engine_root_from_package() -> Path:
    ...
    # verification/ai_provider_configuration/runner.py -> parents[2] == engine/
...
def _default_verification_dir(engine_root: Path) -> Path:
    return engine_root / "verification" / "ai_provider_configuration"


def _default_output_dir(engine_root: Path) -> Path:
    return engine_root / "reports" / "verification" / "sv11-3"


def run_ai_provider_configuration_verification(
    *,
    engine_root: Path | None = None,
    output_dir: Path | None = None,
    write_markdown: bool = True,
) -> ConfigurationVerificationReport:
    """Run the full SV.11.3 verification suite and write the report."""
    ...
    _collect("provider_selection", provider_selection.run_provider_selection_checks())
    _collect("model_resolution", model_resolution.run_model_resolution_checks())
    _collect("credentials", credentials.run_credential_checks())
    _collect(
        "adapter_configuration", adapter_configuration.run_adapter_configuration_checks()
    )
    ...
    if leaks:
        raise RuntimeError(
            "configuration verification report contains forbidden tokens "
...
__all__ = ["engine_root_from_package", "run_ai_provider_configuration_verification"]
