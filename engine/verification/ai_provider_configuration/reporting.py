"""Assemble, sanitize, and verdict the SV.11.3 configuration verification report."""
...
from verification.ai_provider_configuration.models import (
    CheckResult,
...
def compute_verdict(
    ...
    if failed_checks or failed_scenarios:
        return "fail"
    # PASS_WITH_LIMITATIONS is expected/acceptable: the configuration domain
    # is an intentionally unwired representation (see EXPECTED_LIMITATIONS).
...
def assemble_report(
    *,
    ...
    warnings: list[str] = [
        "codestrata.ai.provider_contracts.configuration_* is not called by any product path; "
        "this verification exercises the package directly, not through codestrata assess.",
        "No real provider network calls or credentials are used anywhere in this suite.",
    ]
    notes: list[str] = [
        "CodeStrata v0.2.0 Epic 11, Slice 11.3 — Standardized Provider and Model "
        "Configuration. Defines an unwired, privacy-preserving representation of "
        "resolved provider/model configuration for a possible future common provider "
        "platform.",
        "Does not migrate OpenAI or Bedrock, does not add OpenRouter, does not wire to the "
        "AIProvider protocol, and does not change provider selection defaults, model "
        "resolution precedence, config keys, timeouts, retries, doctor, CLI, or execution.",
        "Slice 11.4+ is not started by this package.",
    ]
