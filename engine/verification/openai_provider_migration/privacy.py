...
1. *Adapter privacy* — the adapter's own diagnostic views, results, reprs, and
   serializations never carry a credential, a base URL, prompt text, response
   text, a provider request ID, raw exception text, a filesystem path, or a
   custom model value.
2. *Report privacy* — the finished SV.11.6 report body carries none of those
   either. That check runs against the assembled report in
   :mod:`verification.openai_provider_migration.reporting`.
"""
...
from codestrata.ai.provider_adapters.openai.client import resolve_client
...
# Every synthetic secret injected into the adapter for these checks. None of
# them may appear in any diagnostic view, result, repr, or report body.
SECRET_API_KEY = "sk-synthetic-privacy-key-0123456789abcdef"  # noqa: S105 - synthetic
SECRET_BASE_URL = "https://synthetic-private-gateway.invalid/v1"
SECRET_KEY_ENV_NAME = "SYNTHETIC_PRIVACY_KEY_VAR"
...
SECRET_REQUEST_ID = "req_synthetic_private_identifier"
...
# environment *variable name* is deliberately excluded: naming it (never its
# value) is how the adapter and doctor tell an operator what to configure.
...
def _secret_settings() -> OpenAISettings:
    return OpenAISettings(api_key_env=SECRET_KEY_ENV_NAME, base_url=SECRET_BASE_URL)


def _secret_adapter(outcome: Any = None) -> Any:
    return build_openai_provider(
        openai_settings=_secret_settings(),
        client=fixtures.Client(outcome if outcome is not None else _secret_response()),
        environment_reader=lambda _name: SECRET_API_KEY,
    )


def _secret_response() -> fixtures.Response:
    return fixtures.response(SECRET_RESPONSE_TEXT, id=SECRET_REQUEST_ID)
...
def check_the_adapter_diagnostic_view_is_clean() -> CheckResult:
    ...
    return CheckResult(
        name="the_adapter_diagnostic_view_leaks_no_secret_url_prompt_or_model_value",
        category="privacy",
        ok=not leaked,
...
def check_the_failure_result_view_is_clean() -> CheckResult:
    adapter = _secret_adapter(fixtures.sdk_exception("BadRequestError", SECRET_EXCEPTION_TEXT))
...
def check_the_configuration_repr_and_view_are_clean() -> CheckResult:
    runtime = build_runtime_configuration(openai_settings=_secret_settings())
    rendered = " ".join(
        (
            repr(runtime),
    ...
    leaked = leaked_substrings(rendered)
    return CheckResult(
        name="the_configuration_repr_and_redacted_view_leak_no_base_url",
        category="privacy",
        ok=not leaked,
        detail=f"leaked={leaked}",
    )
...
def check_the_client_handle_repr_is_clean() -> CheckResult:
    resolution = resolve_client(
        build_runtime_configuration(openai_settings=_secret_settings()).client_inputs,
        injected_client=fixtures.Client(_secret_response()),
    )
    leaked = leaked_substrings(repr(resolution.handle))
    return CheckResult(
        name="the_client_handle_repr_leaks_no_credential_or_base_url",
        category="privacy",
        ok=not leaked,
        detail=f"leaked={leaked}",
    )
...
def check_the_report_body_is_clean(report_body: dict[str, Any]) -> CheckResult:
    """The assembled SV.11.6 report body carries none of the forbidden values."""

    leaked = leaked_substrings(canonical_json(report_body))
    return CheckResult(
        name="the_verification_report_body_leaks_no_secret_prompt_response_or_path",
        category="privacy",
        ok=not leaked,
        detail=f"leaked={leaked}",
...
def check_the_report_body_records_no_timestamp(report_body: dict[str, Any]) -> CheckResult:
    """A timestamp would make two runs differ; determinism forbids one."""
...
def run_adapter_privacy_checks() -> tuple[list[CheckResult], dict[str, Any]]:
    ...
    matrix: dict[str, Any] = {
        ...
        "environment_variable_names_are_reportable": True,
        "environment_variable_values_are_reportable": False,
    }
