"""SV.11.5 Provider Usage Metadata and Capability Discovery verification — orchestration runner."""
...
from verification.ai_provider_capabilities.models import CapabilityVerificationReport, CheckResult


def engine_root_from_package() -> Path:
    """Resolve the Engine repository root (directory containing pyproject.toml)."""

    # verification/ai_provider_capabilities/runner.py -> parents[2] == engine/
...
def _default_verification_dir(engine_root: Path) -> Path:
    return engine_root / "verification" / "ai_provider_capabilities"


def _default_output_dir(engine_root: Path) -> Path:
    return engine_root / "reports" / "verification" / "sv11-5"


def run_ai_provider_capability_verification(
    *,
    engine_root: Path | None = None,
    output_dir: Path | None = None,
    write_markdown: bool = True,
) -> CapabilityVerificationReport:
    """Run the full SV.11.5 verification suite and write the report."""
    ...
    _collect("catalogs", catalogs.run_catalog_checks())
    _collect("validation", validation.run_validation_checks())
    _collect("serialization", serialization.run_serialization_checks())
    _collect("privacy", privacy.run_privacy_checks())
    _collect("compatibility", compatibility.run_compatibility_checks())
    ...
    if leaks:
        raise RuntimeError(
            "capability verification report contains forbidden tokens "
...
__all__ = ["engine_root_from_package", "run_ai_provider_capability_verification"]
