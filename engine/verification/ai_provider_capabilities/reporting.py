"""Assemble, sanitize, and verdict the Provider Usage Metadata and Capability Discovery report."""
...
from verification.ai_provider_capabilities.models import (
...
def compute_verdict(
    ...
    if failed_checks or failed_scenarios:
        return "fail"
    # PASS_WITH_LIMITATIONS is expected/acceptable: capability discovery and
    # usage metadata are an intentionally unwired foundation (see
    # EXPECTED_LIMITATIONS).
...
def assemble_report(
    *,
    ...
    warnings: list[str] = [
        "codestrata.ai.provider_contracts capability_*/usage_* modules are not called by any "
        "product path; this verification exercises the package directly, not through "
        "codestrata assess.",
        "No real provider network calls or credentials are used anywhere in this suite.",
    ]
    notes: list[str] = [
        "CodeStrata v0.2.0 Epic 11, Slice 11.5 — Provider Usage Metadata and Capability "
        "Discovery. Defines unwired, SDK-free capability profiles for bedrock/openai and "
        "extends ProviderUsageMetadata with an optional completion_status field.",
        "Does not migrate OpenAI or Bedrock, does not add OpenRouter, and does not wire "
        "capability discovery or usage metadata into the assessment product path.",
        "Does not modify runtime provider behavior, timeouts, retries, authentication, "
        "error handling, doctor, or codestrata assess selection/defaults/config.",
    ]
