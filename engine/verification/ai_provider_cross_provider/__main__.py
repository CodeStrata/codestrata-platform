"""CLI entry: ``python -m verification.ai_provider_cross_provider``."""
...
from verification.ai_provider_cross_provider.contract import REPORT_FILENAME, REPORT_MD_FILENAME
from verification.ai_provider_cross_provider.reporting import report_directory, verdict_is_allowed
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "(CodeStrata v0.2.0 Epic 11, Slice 11.8)"
        ),
    )
    ...
    print(f"SV.11.8 Cross-Provider Contract Verification: {report.verdict}")
    print(f"checks={report.check_counts} scenarios={len(report.negative_scenarios)}")
    print(f"registry_decision={report.registry_decision} ({report.registry_decision_label})")
