"""Read-only CLI for internal false-negative tracking.

Usage::

    python -m validation.false_negatives list
    python -m validation.false_negatives show <fn-id>
...
from codestrata.domain.quality_metrics.false_negatives import FalseNegativeRecord
from validation.false_negatives.adjudication import ADJUDICATIONS_DIR
...
def _filter_fns(
    ...
    *,
    ...
    tracking = aggregate_false_negative_tracking(records)
    ...
    if rule:
        items = [
            item
            for item in items
            if item.get("expected_rule_id") == rule
            or item.get("expected_signal_id") == rule
            or item.get("expected_entity_id") == rule
        ]
    if entity_type:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m validation.false_negatives",
        description="Internal false-negative tracking (read-only).",
    )
    parser.add_argument(
        "--records-dir",
        type=Path,
    ...
    list_p = sub.add_parser("list", help="List tracked false negatives")
    list_p.add_argument("--repository")
    list_p.add_argument("--area")
    list_p.add_argument("--rule")
    list_p.add_argument("--entity-type")
    list_p.add_argument("--status")
    list_p.add_argument("--classification")
    list_p.add_argument("--open-only", action="store_true")

    show_p = sub.add_parser("show", help="Show one false negative by id")
    show_p.add_argument("fn_id")

    sub.add_parser("summarize", help="Summarize false-negative tracking counts")
    ...
    if args.command == "list":
        ...
        if args.json:
            ...
            for item in items:
                print(
                    f"{item.get('false_negative_id')} | {item.get('repository_id')} | "
    ...
    if args.command == "show":
        ...
        match = next(
            (item for item in items if item.get("false_negative_id") == args.fn_id),
            None,
        )
        if match is None:
            if not str(args.fn_id).startswith("fn:"):
                print("false_negative_id must start with fn:", file=sys.stderr)
                return 2
            print(f"not found: {args.fn_id}", file=sys.stderr)
            return 1
        FalseNegativeRecord.model_validate(match)  # schema check
    ...
    if args.command == "summarize":
        ...
        if args.json:
            ...
            print(
                ...
                f"unsupported={tracking.unsupported_capability} "
                f"insufficient_evidence={tracking.insufficient_evidence} "
