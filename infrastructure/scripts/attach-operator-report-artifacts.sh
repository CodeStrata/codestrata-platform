#!/usr/bin/env bash
# Slice 17.16 — Owner/IAM-admin attach of CodeStrataOperatorReportArtifacts.
#
# Grants S3 CreateBucket + IAM policy/attach for the private report artifact
# store. Run with an IAM principal that can CreatePolicy + AttachUserPolicy.
...
POLICY_DOC="${ROOT}/infrastructure/bootstrap/github-oidc/operator-iam-report-artifacts-policy.json"
POLICY_NAME="CodeStrataOperatorReportArtifacts"
...
if aws iam get-policy --policy-arn "${POLICY_ARN}" >/dev/null 2>&1; then
  ...
  aws iam create-policy \
    ...
    --policy-document "file://${POLICY_DOC}" \
    --description "CodeStrata operator Slice 17.16 private report artifact store." \
    >/dev/null
fi
...
echo "attach-operator-report-artifacts=ok"
echo "Next (as codestrata_infra):"
echo "  ./infrastructure/scripts/apply-report-artifacts.sh"
