"""Tests for Slice 17.25 Community Status Workflow Cleanup."""
...
from verification.community_status_workflow_cleanup.determinism import reports_byte_identical
from verification.community_status_workflow_cleanup.helpers import (
    dict_to_canonical_json,
    report_text_is_safe,
)
from verification.community_status_workflow_cleanup.runner import build_report, write_report


def test_contract_boundary() -> None:
    c = default_contract()
    assert c.start_slice_17_25 is True
    assert c.start_slice_17_26 is False
    assert SCHEMA_NAME == "community-status-workflow-cleanup-verification"
    assert SCHEMA_VERSION == "1.0.0"
    assert SUITE_ID == "sv17-25"
    assert POLICY_REQUIRED_VALUES["start_slice_17_25"] is True
    assert POLICY_REQUIRED_VALUES["start_slice_17_26"] is False
    assert EXPECTED_17_24_PACKAGE == "verification/community_insights_production_auth"
    assert EXPECTED_17_25_PACKAGE == "verification/community_status_workflow_cleanup"
    assert "verification/community_epic17_completion" in SLICE_17_26_PACKAGE_CANDIDATES
    assert "verification/community_release_epic" in SLICE_17_26_PACKAGE_CANDIDATES
    assert "verification/community_marketplace_publish" in SLICE_17_26_PACKAGE_CANDIDATES
...
def test_policy_and_contract_files() -> None:
    ...
    assert (root / EXPECTED_17_25_PACKAGE).is_dir()
...
def test_build_report() -> None:
    ...
    assert report.slice == "17.25"
    assert report.suite_id == SUITE_ID
    assert report.epic17_boundary["start_slice_17_25"] is True
    assert report.epic17_boundary["start_slice_17_26"] is False
...
def test_determinism_and_write() -> None:
    ...
    assert path.name == "community-status-workflow-cleanup-verification.json"
    assert "sv17-25" in path.as_posix()
