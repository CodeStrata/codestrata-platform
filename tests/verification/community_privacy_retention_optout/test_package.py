"""Smoke tests for Slice 18.3 Privacy + Retention + Opt-Out."""
...
from verification.community_privacy_retention_optout.runner import build_report, write_report


def test_contract_boundary() -> None:
    c = default_contract()
    assert c.start_slice_18_3 is True
    assert c.start_slice_18_4 is False
    assert SCHEMA_NAME == "community-privacy-retention-optout-verification"
    assert SCHEMA_VERSION == "1.0.0"
    assert SUITE_ID == "sv18-3"
    assert POLICY_REQUIRED_VALUES["start_slice_18_4"] is False
...
def test_policy_and_contract_files() -> None:
    ...
    assert "community-privacy-retention-optout-verification" in contract.get("$id", "")
...
def test_build_report_smoke() -> None:
    ...
    assert report.slice == "18.3"
    assert report.suite_id == SUITE_ID
    assert report.epic18_boundary["start_slice_18_3"] is True
    assert report.epic18_boundary["start_slice_18_4"] is False
    assert report.freeze_confirmations["slice_18_4_not_started"] is True
...
def test_determinism_and_write() -> None:
    ...
    assert path.name == "community-privacy-retention-optout-verification.json"
    assert "sv18-3" in path.as_posix()


def test_runner_import() -> None:
    from verification.community_privacy_retention_optout.runner import main
