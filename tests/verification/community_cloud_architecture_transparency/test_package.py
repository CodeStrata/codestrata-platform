"""Smoke tests for Slice 18.5 Community Cloud Architecture Transparency."""
...
from verification.community_cloud_architecture_transparency.runner import (
    build_report,
    write_report,
)
...
def test_contract_boundary() -> None:
    c = default_contract()
    assert c.start_slice_18_5 is True
    assert c.start_slice_18_6 is False
    assert SCHEMA_NAME == "community-cloud-architecture-transparency-verification"
    assert SCHEMA_VERSION == "1.0.0"
    assert SUITE_ID == "sv18-5"
    assert POLICY_REQUIRED_VALUES["start_slice_18_6"] is False
...
def test_policy_and_contract_files() -> None:
    ...
    assert "community-cloud-architecture-transparency-verification" in contract.get(
        "$id", ""
    )
...
def test_build_report_smoke() -> None:
    ...
    assert report.slice == "18.5"
    assert report.suite_id == SUITE_ID
    assert report.epic18_boundary["start_slice_18_5"] is True
    assert report.epic18_boundary["start_slice_18_6"] is False
...
def test_determinism_and_write() -> None:
    ...
    assert path.name == "community-cloud-architecture-transparency-verification.json"
    assert "sv18-5" in path.as_posix()


def test_runner_import() -> None:
    from verification.community_cloud_architecture_transparency.runner import main
