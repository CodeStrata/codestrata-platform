...
from aimf.application.evidence.dependency.gradle_collector import parse_gradle_text
from aimf.application.evidence.dependency.maven_collector import parse_pom_text
...
from aimf.application.evidence.dependency.python_collector import (
    parse_pyproject_text,
    parse_requirements_text,
)
from aimf.application.evidence.dependency.service import DependencyEvidenceService
from aimf.config import load_settings
from aimf.config.settings import DependencyEvidenceSettings
...
from aimf.domain.evidence.dependency.models import AggregatedDependencyEvidence
...
def test_manifest_discovery_and_exclusions() -> None:
    ...
    assert ".aimf/workspace/pom.xml" not in selected
    assert excluded >= 1
    assert is_ignored_path(".aimf/workspace/x")
...
def test_service_collect_deterministic_and_artifact(tmp_path: Path) -> None:
    ...
    paths = paths + (".aimf/workspace/pom.xml",)
    texts = dict(texts)
    texts[".aimf/workspace/pom.xml"] = "<project></project>"
