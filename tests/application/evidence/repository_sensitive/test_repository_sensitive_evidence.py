...
from aimf.application.evidence.repository_sensitive.values import (
    REDACTED_PREVIEW,
...
def test_pem_and_openssh_and_certificate_signatures() -> None:
    ...
    assert (
        ContentClassification.CREDENTIAL_ENTRIES
        in classify_text_signatures("AIMF_GITHUB_TOKEN=x\n")
    )
    assert classify_text_signatures("# AIMF_GITHUB_TOKEN=\nAIMF_LOG_LEVEL=WARNING\n") == (
        ContentClassification.NO_SUPPORTED_SENSITIVE_SIGNATURE,
    )
...
def test_no_security_domain_import_in_collectors() -> None:
    import aimf.application.evidence.repository_sensitive.collector as collector_mod
    import aimf.application.evidence.repository_sensitive.service as service_mod

    for module in (collector_mod, service_mod):
        source = Path(module.__file__).read_text(encoding="utf-8")
        assert "aimf.domain.security" not in source
