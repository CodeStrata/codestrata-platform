...
from aimf.config import AimfSettings, KnowledgeAnsweringSettings, KnowledgeRetrievalSettings
from aimf.config.settings import KnowledgeEmbeddingSettings, McpSettings, McpToolsSettings
from aimf.domain.knowledge import VectorRecord
...
@pytest.mark.skipif(
    not (
        ...
        or __import__("os").environ.get("AIMF_PGVECTOR_URL", "").strip()
    ),
    reason="CODESTRATA_DATABASE_URL / AIMF_PGVECTOR_URL not set",
)
