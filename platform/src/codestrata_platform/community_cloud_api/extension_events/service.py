"""Extension event ingestion application service (Slice 7.10)."""
...
from codestrata_platform.community_cloud_api.extension_events.diagnostics import (
    ExtensionEventDiagnostics,
)
...
@dataclass
class IngestExtensionEvent:
    """Orchestrate identity, sink acceptance, and deterministic acknowledgement."""

    sink: ExtensionEventSink = field(default_factory=UnavailableExtensionEventSink)
    lookup: EventIdentityLookup | None = None
    recorder: EventIdentityRecorder | None = None
    extension_policy: CommunityExtensionEventPolicy = field(
        default_factory=default_extension_event_policy
    )
    ...
    last_diagnostics: ExtensionEventDiagnostics | None = None

    def handle(self, context: RequestContext) -> Response:
        request = context.validated_request
        if not isinstance(request, ExtensionEventRequest):
        ...
        try:
            ...
            self._emit_safe(
                LogEventType.EXTENSION_EVENT_REJECTED,
                context,
                status_code=500,
    ...
    def _ingest(self, context: RequestContext, request: ExtensionEventRequest) -> Response:
        self._emit_safe(
            LogEventType.EXTENSION_EVENT_RECEIVED,
            context,
            source_event_type=EXTENSION_EVENT_SOURCE_TYPE,
            extension_schema_version=request.schema_version,
            extension_policy_version=self.extension_policy.policy_version,
            canonical_operation=request.event.operation,
            lifecycle=request.event.lifecycle,
            result=request.event.result,
        )

        limitations = list(self.extension_policy.limitations)
        ...
        if self.lookup is None or self.recorder is None:
            ...
            self._emit_safe(
                ...
                context,
                status_code=503,
                error_code=ERROR_EXTENSION_EVENT_IDENTITY_UNAVAILABLE,
                source_event_type=EXTENSION_EVENT_SOURCE_TYPE,
        ...
        if decision.status is RetryStatus.CONFLICTING_RETRY:
            ...
            self._emit_safe(
                LogEventType.EXTENSION_EVENT_CONFLICT,
                context,
                status_code=409,
        ...
        if decision.status is RetryStatus.EXACT_RETRY:
            response = ExtensionEventResponse(
                status=ExtensionEventIngestionStatus.ALREADY_ACCEPTED,
                safe_event_reference=decision.safe_event_reference,
                retry_status=decision.status.value,
            ...
            self._emit_safe(
                LogEventType.EXTENSION_EVENT_RETRY,
                context,
                status_code=200,
                safe_event_reference=decision.safe_event_reference,
                source_event_type=EXTENSION_EVENT_SOURCE_TYPE,
        ...
        event = ValidatedExtensionEvent(
            ...
            identity_policy_version=self.identity_policy.policy_version,
            extension_event_policy_version=self.extension_policy.policy_version,
            operation_catalog_version=self.extension_policy.operation_catalog_version,
        )
        ...
        if sink_result.status is ExtensionEventSinkStatus.UNAVAILABLE:
            return self._sink_unavailable(context, request, decision, limitations)

        if sink_result.status is ExtensionEventSinkStatus.REJECTED:
            self.last_diagnostics = self._diag(
                True,
                decision.status.value,
            ...
            self._emit_safe(
                ...
                context,
                status_code=422,
                error_code=ERROR_EXTENSION_EVENT_REJECTED,
                safe_event_reference=decision.safe_event_reference,
                source_event_type=EXTENSION_EVENT_SOURCE_TYPE,
                retry_status=decision.status.value,
                extension_schema_version=request.schema_version,
        ...
        try:
            ...
            self._emit_safe(
                ...
                context,
                status_code=503,
                error_code=ERROR_EXTENSION_EVENT_RECORD_FAILED,
                safe_event_reference=decision.safe_event_reference,
                source_event_type=EXTENSION_EVENT_SOURCE_TYPE,
                retry_status=decision.status.value,
                identity_policy_version=self.identity_policy.policy_version,
        ...
        response = ExtensionEventResponse(
            status=ExtensionEventIngestionStatus.ACCEPTED,
            safe_event_reference=decision.safe_event_reference,
            retry_status=decision.status.value,
            schema_version=request.schema_version,
        )
        ...
        self._emit_safe(
            LogEventType.EXTENSION_EVENT_ACCEPTED,
            context,
            status_code=202,
            safe_event_reference=decision.safe_event_reference,
            source_event_type=EXTENSION_EVENT_SOURCE_TYPE,
    ...
    def _sink_unavailable(
        ...
        self.last_diagnostics = self._diag(
            ...
            ExtensionEventSinkStatus.UNAVAILABLE.value,
            "skipped",
            safe_ref,
            limitations,
        )
        self._emit_safe(
            ...
            context,
            status_code=503,
            error_code=ERROR_EXTENSION_EVENT_SINK_UNAVAILABLE,
    ...
    def _diag(
        ...
        return ExtensionEventDiagnostics(
            ...
            safe_event_reference=safe_ref,
            schema_version=self.extension_policy.schema_version,
            policy_version=self.extension_policy.policy_version,
            operation_catalog_version=self.extension_policy.operation_catalog_version,
    ...
    def _emit_safe(
        ...
        *,
        ...
        if extension_schema_version is not None:
            fields["extension_schema_version"] = extension_schema_version
        if extension_policy_version is not None:
            fields["extension_policy_version"] = extension_policy_version
