...
from codestrata_platform.knowledge_graph.cli.enterprise_mapping import (
    EXIT_BLOCKED,
    EXIT_ERROR,
    EXIT_SUCCESS,
    dumps_json,
...
from codestrata_platform.knowledge_graph.cli.enterprise_rendering import (
    emit_build,
    emit_diff,
    emit_graph,
    emit_validation,
...
def _policy_for_build(settings: CodestrataSettings) -> EnterprisePolicy:
...
def inspect_command(
    ...
    enterprise_id: Annotated[str, typer.Option("--enterprise-id")] = "enterprise:example",
    as_json: Annotated[bool, typer.Option("--json")] = False,
...
def query_command(
    ...
    try:
        if subject == "applications" and domain:
            ...
            typer.secho(
                "Unsupported query; see `codestrata enterprise query --help`",
                fg=typer.colors.RED,
                err=True,
            )
