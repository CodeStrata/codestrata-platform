"""AI usage Platform-only boundary guards."""
...
from codestrata_platform.community_cloud_api.ai_usage import COMMUNITY_AI_USAGE_POLICY_URN
...
def test_production_routes_six() -> None:
    paths = {
        (r.method, r.path)
...
def test_schema_constants_unchanged() -> None:
    ...
    assert COMMUNITY_EXTENSION_EVENT_SCHEMA_VERSION == "1.0"
    assert COMMUNITY_EXTENSION_EVENT_POLICY_VERSION == "1.0"
    assert COMMUNITY_EXTENSION_OPERATION_CATALOG_VERSION == "1.0"
    assert COMMUNITY_AI_USAGE_SCHEMA_VERSION == "1.0"
    assert COMMUNITY_AI_USAGE_POLICY_VERSION == "1.0"
    assert COMMUNITY_AI_CAPABILITY_CATALOG_VERSION == "1.0"
    assert COMMUNITY_AI_PROVIDER_FAMILY_CATALOG_VERSION == "1.0"
    assert COMMUNITY_AI_MODEL_FAMILY_CATALOG_VERSION == "1.0"
