"""Extension event endpoint tests."""
...
from .extension_event_helpers import configured_extension_client, valid_extension_event_body


def test_route_registered() -> None:
    registry = RouteRegistry.foundation_v1()
    spec = registry.get(version="v1", method="POST", path="/extension-events")
    assert spec is not None
    assert spec.name == EXTENSION_EVENTS_ROUTE_NAME


def test_get_body_media_schema_operation() -> None:
    client, *_ = configured_extension_client()
    assert client.get("/api/v1/extension-events").status_code == 405
    assert (
        client.get("/api/v1/extension-events").json()["error"]["code"]
        == ERROR_METHOD_NOT_ALLOWED
    )
    assert (
        client.post("/api/v1/extension-events").json()["error"]["code"]
        == ERROR_BODY_REQUIRED
    )
    media = client.post(
    ...
    schema = client.post(
        "/api/v1/extension-events",
        json=valid_extension_event_body(schema_version="2.0"),
    )
    assert schema.status_code == 422
    assert schema.json()["error"]["code"] == ERROR_UNSUPPORTED_EXTENSION_EVENT_SCHEMA
    op = valid_extension_event_body()
    op["event"] = {**op["event"], "operation": "not_a_real_op"}  # type: ignore[dict-item]
    rejected = client.post("/api/v1/extension-events", json=op)
    assert rejected.status_code == 422
    assert rejected.json()["error"]["code"] == ERROR_UNSUPPORTED_EXTENSION_OPERATION


def test_first_accepted_independent_sinks() -> None:
    client, sink, store, _, tel, meta, cli = configured_extension_client()
    ...
    expected = ExtensionEventResponse(
...
def test_defaults_fail_closed() -> None:
    ...
    response = client.post("/api/v1/extension-events", json=valid_extension_event_body())
    assert response.status_code == 503
    assert response.json()["error"]["code"] == ERROR_EXTENSION_EVENT_IDENTITY_UNAVAILABLE

    store = InMemoryEventIdentityStore()
    client2 = TestClient(
        create_community_cloud_app(
            extension_event_sink=UnavailableExtensionEventSink(),
    ...
    response2 = client2.post(
        "/api/v1/extension-events", json=valid_extension_event_body()
    )
    assert response2.status_code == 503
    assert response2.json()["error"]["code"] == ERROR_EXTENSION_EVENT_SINK_UNAVAILABLE
