"""Extension event-stream envelope construction tests (Slice 8.3)."""
...
from codestrata_platform.community_cloud_api.data_lake.streams.extension_events import (
    SCHEMA_NAME,
...
from ._request_test_helpers import make_extension_event_request, make_telemetry_request
...
def test_registered_contract_matches_module_constants() -> None:
    ...
    assert descriptor.policy_id == "community-extension-event-policy:1.0"
    assert descriptor.allowed_client_types == frozenset(ALLOWED_EXTENSION_CLIENTS)


def test_build_data_lake_envelope_end_to_end_for_extension_event() -> None:
    envelope = build_data_lake_envelope(
        event_stream="extension_event",
        request=make_extension_event_request(),
        event_key="event:extevent000000000000",
        safe_event_reference="evt-ext0001",
        clock=_CLOCK,
    )
    assert envelope.payload["event"]["operation"] == "assess"
    assert envelope.source_contract.schema_name == "community-extension-event"


def test_build_data_lake_envelope_rejects_a_telemetry_request_for_extension_stream() -> None:
