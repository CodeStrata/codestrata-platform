"""Extension event partition/diagnostics privacy boundary tests (Slice 8.7)."""
...
# for a default extension event submission. ``vscode_extension`` is
# deliberately excluded here — it IS an approved S3-metadata / diagnostics
# value (Option B). Do not assert it absent from metadata.
_SENSITIVE_VALUE_FRAGMENTS = (
    "ext-evt-0001",  # event_id
    "assess",  # operation
    "completed",  # lifecycle
    "succeeded",  # result
)
...
def _result(**overrides: object):
    return project_extension_event_storage_object(extension_event_envelope(**overrides))
...
def test_s3_metadata_has_client_type_only_as_extra() -> None:
    ...
    assert metadata["codestrata-client-type"] == "vscode_extension"


def test_s3_metadata_has_no_editor_operation_lifecycle_or_identity_keys() -> None:
    metadata = _result().storage_object.to_s3_metadata()
    for forbidden_key in (
        "codestrata-editor",
        "codestrata-operation",
        "codestrata-extension-operation",
...
def test_diagnostics_includes_client_type_and_operation_catalog_version() -> None:
    diagnostics = _result().diagnostics.to_stable_dict()
...
def test_diagnostics_never_contains_operation_editor_lifecycle_or_result() -> None:
    diagnostics = _result().diagnostics.to_stable_dict()
    assert "operation" not in diagnostics
    assert "editor" not in diagnostics
...
def test_diagnostics_never_contains_workspace_or_document_fields() -> None:
    diagnostics = _result().diagnostics.to_stable_dict()
    for forbidden_key in (
        "workspace_path",
        "workspace_state",
