...
def test_app_and_wiring_have_no_access_policy_or_data_lake() -> None:
    for path in (APP_PY, DEPLOYMENT_WIRING):
        text = path.read_text(encoding="utf-8")
        assert "access_policy" not in text
        assert "data_lake" not in text
        assert "CommunityDataLakeAccessPolicy" not in text
        assert "writer_policy_attached" not in text
        assert "ListBucket" not in text
