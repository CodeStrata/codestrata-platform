"""Extension event partition policy tests (Slice 8.7).
...
only ``default_extension_event_partition_policy()``'s concrete shape.
"""
...
def test_default_extension_event_policy_ids_and_version() -> None:
    policy = default_extension_event_partition_policy()
    assert policy.policy_id == EXTENSION_EVENT_PARTITION_POLICY_ID
    assert policy.policy_version == EXTENSION_EVENT_PARTITION_POLICY_VERSION
    assert policy.policy_version == "1.0"
    assert policy.policy_token == EXTENSION_EVENT_PARTITION_POLICY_URN
    assert EXTENSION_EVENT_PARTITION_POLICY_URN == (
        "community-extension-event-partition-policy:1.0"
    )


def test_default_extension_event_policy_event_stream() -> None:
    assert default_extension_event_partition_policy().event_stream is EventStream.EXTENSION_EVENT


def test_default_extension_event_policy_supported_versions() -> None:
    policy = default_extension_event_partition_policy()
    ...
    assert policy.supported_source_policy_ids == frozenset({COMMUNITY_EXTENSION_EVENT_POLICY_URN})
    assert policy.supported_source_policy_ids == frozenset(
        {"community-extension-event-policy:1.0"}
    )


def test_default_extension_event_policy_has_no_assessment_schema_versions() -> None:
    policy = default_extension_event_partition_policy()
    assert policy.supported_assessment_schema_versions == frozenset()


def test_default_extension_event_policy_supported_operation_catalog_versions() -> None:
    policy = default_extension_event_partition_policy()
...
def test_default_extension_event_policy_required_dimensions_are_generic_only() -> None:
    policy = default_extension_event_partition_policy()
    assert policy.required_path_dimensions == ("stream", "schema_version", "year", "month", "day")
    assert policy.optional_path_dimensions == ()


def test_default_extension_event_policy_s3_metadata_allowlist_includes_client_type() -> None:
    policy = default_extension_event_partition_policy()
...
def test_default_extension_event_policy_forbidden_fields_cover_private_payload_material() -> None:
    policy = default_extension_event_partition_policy()
    for field_name in (
        "event_id",
        "installation_id",
...
def test_default_extension_event_policy_forbidden_fields_never_overlap_required_dimensions() -> None:
    policy = default_extension_event_partition_policy()
    assert not (set(policy.forbidden_partition_fields) & set(policy.required_path_dimensions))


def test_default_extension_event_policy_limitations_document_the_dimension_decision() -> None:
    policy = default_extension_event_partition_policy()
    ...
    assert "editor_remains_private_payload" in blob


def test_default_extension_event_policy_max_partition_depth_covers_required_dimensions() -> None:
    policy = default_extension_event_partition_policy()
    assert policy.max_partition_depth >= len(policy.required_path_dimensions)


def test_default_extension_event_policy_is_deterministic_across_calls() -> None:
...
def test_default_extension_event_policy_to_stable_dict_includes_operation_catalog_versions() -> None:
    policy = default_extension_event_partition_policy()
