"""Slice 8.8 boundary tests: wiring isolation + low-level partition-vs-policy checks.

Mirrors ``test_extension_event_partition_boundary.py``'s structure for the
``ai_usage`` stream's new module. Generic ``stream_partitions.py``
mechanics are already exhaustively covered by
``test_assessment_metadata_partition_boundary.py`` — this module focuses on
what is new for Slice 8.8: the ``ai_usage_partitioning.py`` module's
existence/boto3-free status, wiring isolation for its new symbols
(including in ``app.py``, AI usage packages, and the Engine), and
AI-usage-specific partition-key checks.
"""
...
def _envelope():
    return ai_usage_envelope(event_key="event:ai-usage-boundary-key")
...
def test_ai_usage_partitioning_module_exists_under_data_lake_streams() -> None:
    assert (DATA_LAKE_PKG / "streams" / "ai_usage_partitioning.py").is_file()


def test_ai_usage_partitioning_module_has_no_boto3_import() -> None:
    path = DATA_LAKE_PKG / "streams" / "ai_usage_partitioning.py"
...
def test_production_wiring_files_have_no_reference_to_new_slice_8_8_symbols() -> None:
    targets = (
...
def test_ai_usage_endpoint_service_has_no_reference_to_data_lake() -> None:
    ai_usage_pkg = COMMUNITY_CLOUD_API_PKG / "ai_usage"
    for module_name in ("routes.py", "service.py", "ports.py"):
        text = (ai_usage_pkg / module_name).read_text(encoding="utf-8")
        assert "data_lake" not in text, f"{module_name} references data_lake"


def test_engine_package_has_no_reference_to_ai_usage_partitioning() -> None:
    if not ENGINE_SRC.is_dir():  # pragma: no cover - defensive, engine always present
        return
    offenders: list[str] = []
    for path in ENGINE_SRC.rglob("*.py"):
        text = path.read_text(encoding="utf-8")
        if "ai_usage_partitioning" in text or "project_ai_usage_storage_object" in text:
...
def test_assert_partition_dimensions_allowed_rejects_capability_extra_dimension() -> None:
    key = (
        "raw/stream=ai_usage/schema_version=1.0/year=2026/month=08/day=04/"
        "capability=modernization_advisor/x.json"
    )
    with pytest.raises(StreamPartitionError):
        assert_partition_dimensions_allowed(key, POLICY)


def test_assert_partition_dimensions_allowed_rejects_provider_family_extra_dimension() -> None:
    key = (
        "raw/stream=ai_usage/schema_version=1.0/year=2026/month=08/day=04/"
        "provider_family=openai/x.json"
    )
    with pytest.raises(StreamPartitionError):
        assert_partition_dimensions_allowed(key, POLICY)


def test_assert_partition_dimensions_allowed_rejects_model_family_extra_dimension() -> None:
    key = (
        "raw/stream=ai_usage/schema_version=1.0/year=2026/month=08/day=04/"
        "model_family=gpt_family/x.json"
    )
    with pytest.raises(StreamPartitionError):
        assert_partition_dimensions_allowed(key, POLICY)
...
def test_assert_partition_key_matches_policy_happy_path() -> None:
    ...
    assert dims["stream"] == "ai_usage"


def test_assert_partition_key_matches_policy_rejects_path_traversal() -> None:
    envelope = _envelope()
    key = (
        f"raw/stream=ai_usage/schema_version=1.0/year={envelope.accepted_year}/"
...
def test_assert_partition_key_matches_policy_rejects_capability_extra_dimension() -> None:
    envelope = _envelope()
    key = (
        f"raw/stream=ai_usage/schema_version=1.0/year={envelope.accepted_year}/"
        f"month={envelope.accepted_month}/day={envelope.accepted_day}/"
        f"capability=modernization_advisor/x.json"
    )
    with pytest.raises(StreamPartitionError):
        assert_partition_key_matches_policy(key, POLICY, envelope)
...
def test_assert_partition_key_matches_policy_rejects_date_mismatch() -> None:
    envelope = _envelope()
    key = "raw/stream=ai_usage/schema_version=1.0/year=2020/month=01/day=01/x.json"
    with pytest.raises(StreamPartitionError):
        assert_partition_key_matches_policy(key, POLICY, envelope)
...
def test_assert_s3_metadata_matches_policy_accepts_client_type_key() -> None:
    assert_s3_metadata_matches_policy(
        {
            "codestrata-stream": "ai_usage",
            "codestrata-client-type": "codestrata_cli",
...
def test_assert_s3_metadata_matches_policy_rejects_capability_key() -> None:
    with pytest.raises(StreamPartitionError):
        assert_s3_metadata_matches_policy(
            {"codestrata-capability": "modernization_advisor"}, POLICY
        )
