"""End-to-end AI usage storage projection + persistence tests (Slice 8.8).
...
``test_extension_event_storage_projection.py`` (Option B client-type metadata).
"""
...
S3_CONFIG = S3DataLakeStoreConfiguration(bucket_name="codestrata-test-ai-usage-lake")
...
class _StoreWithoutStorageObjectSupport:
    ...
    def put_immutable_event(self, envelope: object) -> object:  # pragma: no cover - unused
        raise AssertionError("should never be called by store_projected_ai_usage")
...
def test_in_memory_store_stores_the_projected_object() -> None:
    envelope = ai_usage_envelope(event_id="ai-usage-proj-imem-001")
    projection = project_ai_usage_storage_object(envelope)
...
def test_in_memory_store_replay_is_already_exists() -> None:
    envelope = ai_usage_envelope(event_id="ai-usage-proj-imem-002")
    projection = project_ai_usage_storage_object(envelope)
    store = InMemoryCommunityDataLakeStore()
    first = store_projected_ai_usage(store, projection)
    second = store_projected_ai_usage(store, projection)
...
def test_in_memory_store_conflict_never_overwrites() -> None:
    ...
    envelope_a = ai_usage_envelope(
        event_key="event:ai-usage-conflict-key", event_id="ai-usage-proj-imem-003a"
    )
    envelope_b = ai_usage_envelope(
        event_key="event:ai-usage-conflict-key", event_id="ai-usage-proj-imem-003b"
    )
    projection_a = project_ai_usage_storage_object(envelope_a)
    projection_b = project_ai_usage_storage_object(envelope_b)
    assert projection_a.storage_object.object_key == projection_b.storage_object.object_key

    store = InMemoryCommunityDataLakeStore()
    first = store_projected_ai_usage(store, projection_a)
    second = store_projected_ai_usage(store, projection_b)
    assert first.status is StorageWriteStatus.STORED
...
def test_in_memory_bytes_stored_match_the_projected_canonical_bytes() -> None:
    envelope = ai_usage_envelope(event_id="ai-usage-proj-imem-004")
    projection = project_ai_usage_storage_object(envelope)
    store = InMemoryCommunityDataLakeStore()
    result = store_projected_ai_usage(store, projection)
...
def test_store_projected_ai_usage_requires_put_immutable_storage_object() -> None:
    envelope = ai_usage_envelope(event_id="ai-usage-proj-imem-005")
    projection = project_ai_usage_storage_object(envelope)
    with pytest.raises(AttributeError):
        store_projected_ai_usage(_StoreWithoutStorageObjectSupport(), projection)
...
def test_s3_store_put_immutable_storage_object_is_stored() -> None:
    envelope = ai_usage_envelope(event_id="ai-usage-proj-s3-001")
    projection = project_ai_usage_storage_object(envelope)
    client = FakeS3Client()
...
def test_s3_store_put_always_sets_if_none_match_star() -> None:
    envelope = ai_usage_envelope(event_id="ai-usage-proj-s3-002")
    projection = project_ai_usage_storage_object(envelope)
    client = FakeS3Client()
    store = _s3_store(client)
    store_projected_ai_usage(store, projection)
    _, kwargs = client.calls[0]
    assert kwargs["IfNoneMatch"] == "*"


def test_s3_store_put_sets_body_content_type_and_checksum() -> None:
    envelope = ai_usage_envelope(event_id="ai-usage-proj-s3-003")
    projection = project_ai_usage_storage_object(envelope)
    client = FakeS3Client()
    store = _s3_store(client)
    store_projected_ai_usage(store, projection)
...
def test_s3_store_object_key_starts_with_generic_accepted_path() -> None:
    envelope = ai_usage_envelope(event_id="ai-usage-proj-s3-004")
    projection = project_ai_usage_storage_object(envelope)
    client = FakeS3Client()
    store = _s3_store(client)
    result = store_projected_ai_usage(store, projection)
    assert result.object_key.startswith("raw/stream=ai_usage/schema_version=1.0/")


def test_s3_store_metadata_includes_client_type() -> None:
    envelope = ai_usage_envelope(event_id="ai-usage-proj-s3-005")
    projection = project_ai_usage_storage_object(envelope)
    client = FakeS3Client()
    store = _s3_store(client)
    result = store_projected_ai_usage(store, projection)
    ...
    assert "capability" not in stored_metadata
    assert "provider_family" not in stored_metadata
...
def test_s3_store_metadata_excludes_event_id_and_installation_id() -> None:
    envelope = ai_usage_envelope(
        event_id="ai-usage-proj-s3-006", installation_id="install-abcdef12"
    )
    projection = project_ai_usage_storage_object(envelope)
    client = FakeS3Client()
    store = _s3_store(client)
    ...
    assert "ai-usage-proj-s3-006" not in blob
...
def test_s3_store_replay_resolves_to_already_exists() -> None:
    envelope = ai_usage_envelope(event_id="ai-usage-proj-s3-007")
    projection = project_ai_usage_storage_object(envelope)
    client = FakeS3Client()
    store = _s3_store(client)
    first = store_projected_ai_usage(store, projection)
...
def test_s3_store_put_immutable_event_alone_does_not_carry_client_type() -> None:
    """Documents the durability defect ``store_projected_ai_usage`` avoids.
    ...
    is exactly why ``store_projected_ai_usage`` requires
    ``put_immutable_storage_object`` instead of falling back to it.
    """

    envelope = ai_usage_envelope(event_id="ai-usage-proj-s3-008")
    client = FakeS3Client()
    store = _s3_store(client)
