"""Architecture guarantees for Executive Presentation."""
...
def test_presentation_application_forbids_disallowed_imports() -> None:
    for path in sorted(PRESENTATION_APPLICATION.rglob("*.py")):
...
def test_presentation_adapter_consumes_executive_intelligence_only() -> None:
    adapter = PRESENTATION_APPLICATION / "adapter.py"
    modules = _imported_modules(adapter)
    assert any(
...
def test_presentation_api_calls_application_only() -> None:
    for path in sorted(PRESENTATION_API.rglob("*.py")):
        for module in _imported_modules(path):
