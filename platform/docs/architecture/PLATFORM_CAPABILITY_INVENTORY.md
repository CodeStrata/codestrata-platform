...
Implementation-based operational inventory for CodeStrata (`codestrata` package/CLI).  
...
| CLI / MCP | `codestrata rules`, shared-rule tools | substantially complete | `cli/rules.py`, `interfaces/mcp/tools/rules.py` | cli/mcp tests | |
...
| CLI / MCP | `codestrata evidence`, evidence tools | substantially complete | `cli/evidence.py`, MCP evidence tools | yes | |
...
| CLI / MCP exposure | `codestrata agent`, `*_with_agents` | substantially complete | cli + MCP | yes | |
| LLM tool agent | `ModernizationAssessmentAgent` + `CodeStrataToolRegistry` | partial | `ai/agents/`, `ai/tools/` | ai tests | Enrichment preferred in assess |
...
Registration: `src/codestrata/cli/__init__.py`.
...
| `CodeStrataToolRegistry` | `ai/tools/registry.py` | AI analysis tools |
...
| Package name `codestrata` vs product CodeStrata | Branding vs implementation |
