...
**Implementation package / CLI:** `codestrata` (not renamed)  
...
| Principle | Evidence |
| --------- | -------- |
| Domain-driven separation | Packages under `src/codestrata/domain/` (graph, findings, rules, evidence, architecture, enterprise, …) vs `application/` vs `services/` vs `interfaces/` |
...
```text
                    ┌─────────────────────────────────────────┐
                    │  CLI (codestrata)  /  MCP (CodeStrata/stdio)  │
...
AI-enhanced assess → enrichment service (preferred)
  (ModernizationAssessmentAgent / CodeStrataToolRegistry still present)
```
...
| CLI | `codestrata rules list|inspect|explain` |
...
CLI: `codestrata evidence …`. MCP: `list_evidence_providers`, `inspect_evidence_provider`, `list_evidence_capabilities`, `explain_evidence_provider`.  
Default: **disabled**.
...
| Concern | Detail |
| ------- | ------ |
| Entry | `codestrata mcp serve` → `interfaces/mcp/server.py` / `factory.py` |
...
3. **CLI?** Yes — `codestrata agent {review,assess,validate,compare,modernization-review}`.  
4. **MCP?** Yes — `*_with_agents` tools.  
5. **Tool-calling?** Application agents: no LLM tools; AI stack: `CodeStrataToolRegistry` exists.  
...
Root: `codestrata` (`src/codestrata/cli/__init__.py`, entry `codestrata = "codestrata.cli:app"`).
...
JSON output flags exist on many inspect commands. Config via `--config` / `codestrata.toml`.

---

## 17. Configuration Platform

Owner: `CodestrataSettings` in `config/settings.py`; load via `load_settings`.
...
| `CodeStrataToolRegistry` | `ai/tools/registry.py` | AI tool registration |
...
| Package/CLI name `codestrata` vs product CodeStrata | `pyproject.toml`, CLI | Naming friction | No | Explicit non-goal |
