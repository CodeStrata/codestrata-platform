...
- **Epic 11, Slice 11.12** is authoritative privacy, failure-isolation, and
  ...
  Slice **13.10** confirms source locality (`community-vscode-source-locality-policy:1.0`; [vscode-plugin/docs/source-locality.md](vscode-plugin/docs/source-locality.md); verification `vscode-source-locality-verification:1.0.0`). Slice **13.11** defines the permanent CLI–extension compatibility matrix (`community-vscode-cli-compatibility-policy:1.0`; [vscode-plugin/docs/cli-compatibility.md](vscode-plugin/docs/cli-compatibility.md); verification `vscode-cli-compatibility-verification:1.0.0`). Slice **13.12** finalizes Marketplace branding (`community-vscode-marketplace-branding-policy:1.0`; [vscode-plugin/docs/marketplace-branding.md](vscode-plugin/docs/marketplace-branding.md); verification `vscode-marketplace-branding-verification:1.0.0`). Slice **13.13** finalizes Marketplace documentation (`community-vscode-marketplace-documentation-policy:1.0`; [vscode-plugin/docs/marketplace-documentation.md](vscode-plugin/docs/marketplace-documentation.md); verification `vscode-marketplace-documentation-verification:1.0.0`). Slice **13.14** validates clean install/update (`community-vscode-clean-install-policy:1.0`; [vscode-plugin/docs/clean-install-update.md](vscode-plugin/docs/clean-install-update.md); verification `vscode-clean-install-verification:1.0.0`). Slice **13.15** is Epic 13 completion verification (`vscode-epic13-completion-verification:1.0.0`). **Epic 13 – VS Code Extension is complete for the CodeStrata v0.2.0 epic scope.** The Community VS Code workflow, CLI discovery/guidance/initialization, assessment, progress, local report opening, recovery, telemetry consent, source-locality, compatibility, Marketplace branding/documentation, and clean-install validation have been completed and verified. Marketplace publication, release tagging, and production deployment remain separate release gates. **Epic 14 – Unified CodeStrata Product Experience:** Slice **14.1** defines the authoritative Visual Design System under [`design-system/`](design-system/) (`codestrata-design-system-policy:1.0`, `codestrata-visual-language-policy:1.0`; verification `codestrata-visual-design-system-verification:1.0.0`). Slice **14.2** rebuilds the Community documentation site on that Design System (`community-documentation-redesign-policy:1.0`; verification `community-documentation-redesign-verification:1.0.0`) — Community-only active navigation; Platform/commercial docs excluded from publish. Slice 14.2 does not redesign Assessment HTML reports, Engineering Intelligence reports, the VS Code extension, or Marketplace assets. Slice **14.3** redesigns Assessment HTML report presentation only (`codestrata-assessment-report-design-policy:1.0`; verification `assessment-html-report-redesign-verification:1.0.0`) — Assessment schema 1.2 and assessment truth unchanged. Slice **14.4** redesigns commercial Engineering Intelligence Report presentation only (`codestrata-engineering-intelligence-report-design-policy:1.0`; verification `engineering-intelligence-report-redesign-verification:1.0.0`) — intelligence truth and Assessment HTML unchanged. Slice **14.5** aligns Community VS Code presentation with Design System 1.0 using native host surfaces (`codestrata-vscode-visual-experience-policy:1.0`; verification `vscode-visual-experience-verification:1.0.0`) — extension remains **0.2.0**. Slice **14.6** aligns Marketplace visual assets only (`codestrata-marketplace-visual-assets-policy:1.0`; verification `marketplace-visual-assets-verification:1.0.0`) — Marketplace copy semantics unchanged; universal logo deferred to Slice 14.10. Slice **14.7** standardizes cross-surface presentation contracts (`codestrata-cross-surface-presentation-policy:1.0`; verification `cross-surface-presentation-verification:1.0.0`) — Design System remains **1.0**. Slice **14.8** standardizes visualization grammar for scores/risk/status/confidence/charts (`codestrata-visualization-policy:1.0`; verification `visualization-system-verification:1.0.0`) — domain scoring/thresholds unchanged; chart foundation defined without inventing dashboards. Slice **14.9** standardizes report information architecture and navigation across the Community Assessment report and the commercial Engineering Intelligence Report (`codestrata-report-information-architecture-policy:1.0`, contract `codestrata-report-information-architecture-contract:1.0`; verification `report-navigation-information-architecture-verification:1.0.0`) — canonical hierarchy roles, heading/anchor/navigation rules, and presence-driven contents; section order, stable deep links, schemas, and visualization semantics unchanged; reports remain distinct products. Slice **14.10** establishes one authoritative CodeStrata brand asset system (`codestrata-brand-asset-policy:1.0`, contracts [`design-system/contracts/assets.json`](design-system/contracts/assets.json) and [`design-system/contracts/icons.json`](design-system/contracts/icons.json); documentation [design-system/documentation/brand-assets.md](design-system/documentation/brand-assets.md); verification `brand-assets-verification:1.0.0`) — masters live under [`design-system/assets/brand/`](design-system/assets/brand/) and every surface derivative (documentation/API-portal marks, favicon, Assessment and EIR inline report marks, VS Code `currentColor` activity icon, 128×128 Marketplace raster icon) is generated deterministically from that master by [`scripts/generate_brand_assets.py`](scripts/generate_brand_assets.py); `governance/assets/` becomes a historical archive rather than a second master; legacy amber, AIMF, and Cursor visual identities are absent from active surfaces; no runtime behavior, command IDs, schemas, report content, report IA, or visualization semantics changed; extension remains **0.2.0**. Slice **14.11** validates responsive and accessible experience acceptance across Docs, Assessment HTML, EIR HTML, VS Code, and Marketplace surfaces (`codestrata-accessibility-responsive-policy:1.0`, contracts [`design-system/contracts/accessibility.json`](design-system/contracts/accessibility.json) and [`design-system/contracts/responsive.json`](design-system/contracts/responsive.json); documentation [design-system/documentation/accessibility-and-responsive.md](design-system/documentation/accessibility-and-responsive.md); verification `responsive-accessibility-verification:1.0.0`) — WCAG 2.2 AA-oriented structural and computed checks, dark-surface token tints, and viewport baselines; no formal certification claim; deployment configuration unchanged. Slice **14.12** validates Community documentation deployment configuration for Cloudflare Static Assets (`codestrata-documentation-deployment-policy:1.0`; [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md); verification `documentation-deployment-verification:1.0.0`) — Wrangler dry-run, build output alignment, and export boundaries; no production deploy during verification. Slice **14.13** validates cross-surface visual consistency across Docs, Assessment HTML, EIR HTML, VS Code, Marketplace, and API portal surfaces (`codestrata-cross-surface-consistency-policy:1.0`, contract [`design-system/contracts/cross-surface-consistency.json`](design-system/contracts/cross-surface-consistency.json); verification `cross-surface-visual-consistency-verification:1.0.0`) — semantic consistency matrix, legacy amber/AIMF/Georgia absent from active surfaces, public token mirrors aligned to Design System 1.0 teal/canvas palette; no runtime behavior, schema, or report IA change; extension remains **0.2.0**. Slice 14.14 not started.
