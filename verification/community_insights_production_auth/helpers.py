"""Helpers for Slice 17.24 verification."""
...
FORBIDDEN_REPORT_PATTERNS = (
    ...
    re.compile(r'"salt_b64"\s*:\s*"[A-Za-z0-9+/=]{8,}"'),
    re.compile(r'"hash_b64"\s*:\s*"[A-Za-z0-9+/=]{8,}"'),
    re.compile(r"cs_insights_session=[A-Za-z0-9._%-]{16,}"),
...
def sanitize_detail(detail: str) -> str:
    ...
    safe = re.sub(r'"salt_b64"\s*:\s*"[^"]+"', '"salt_b64":"[redacted]"', safe)
    safe = re.sub(r'"hash_b64"\s*:\s*"[^"]+"', '"hash_b64":"[redacted]"', safe)
    return safe
