...
from verification.community_cloud_production_recovery.contract import (
    ...
    SCHEMA_NAME,
    SCHEMA_VERSION,
    SV1710_OUTPUT_RELATIVE,
)
from verification.community_cloud_production_recovery.determinism import (
    dict_to_canonical_json,
    report_text_is_safe,
)
from verification.community_cloud_production_recovery.models import Report


def write_report(monorepo: Path, report: Report) -> Path:
    out_dir = monorepo / SV1710_OUTPUT_RELATIVE
    ...
    (out_dir / REPORT_MD).write_text(
        ...
        "Slice 17.10 — production recovery validation (operations). "
        "Slice 17.11 not started. No commit/tag/publish.\n",
        encoding="utf-8",
    )
    return json_path
