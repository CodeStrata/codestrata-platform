"""Safety scans for Slice 12.1 verification reports."""
...
def check_report_safety(report_path: Path) -> tuple[list[CheckResult], list[Defect]]:
    ...
    if not report_path.is_file():
        ...
        defects.append(Defect("harness defect", str(report_path.name), "present", "absent"))
    ...
    if leaks:
        defects.append(
            Defect("harness defect", report_path.name, "no leaks", ",".join(leaks))
        )
    return checks, defects
