...
FORBIDDEN_REPORT_PATTERNS = (
    ...
    re.compile(r"cs_insights_session=[^\s\"';]+"),
    # Packaged public credential value must never appear in evidence output.
    re.compile(r"cscc_v1_[A-Za-z0-9]{20,}"),
)
...
def sanitize_detail(detail: str) -> str:
    ...
    safe = re.sub(r"cscc_v1_[A-Za-z0-9]+", "cscc_v1_[redacted]", safe)
    safe = re.sub(r"cs_insights_session=[^\s\"';]+", "cs_insights_session=[redacted]", safe)
    return safe
