...
from aimf.config.settings import AimfSettings, SecurityAssessmentSectionSettings
...
def security_assessment_section_enabled(settings: AimfSettings | None) -> bool:
    if settings is None:
        return False
    return bool(settings.assessment.sections.security.enabled)


def security_pack_enabled(settings: AimfSettings | None) -> bool:
    if settings is None:
        return False
