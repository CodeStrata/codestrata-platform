...
from aimf.application.acceptance.models import (
    AcceptanceCheckResult,
    AcceptanceHarnessResult,
    RepositoryAcceptanceResult,
)
from aimf.application.acceptance.qa import (
...
from aimf.application.acceptance.settings import build_acceptance_settings
from aimf.application.acceptance.summary import write_acceptance_summaries
from aimf.application.acceptance.targets import (
    ACCEPTANCE_TARGETS,
    LIVE_DETERMINISM_EXTRA_PATHS,
)
from aimf.application.onboarding import OnboardingApplicationService
from aimf.application.report_validation import validate_report_json
from aimf.config.settings import AimfSettings
from aimf.domain.onboarding.errors import OnboardingError
from aimf.reporting.contract.canonical import strip_volatile_fields
...
class MvpAcceptanceService:
    ...
    def _run_repository(
        ...
        *,
        ...
        output_directory: Path,
        config_path: Path,
        base_settings: AimfSettings | None,
    ) -> RepositoryAcceptanceResult:
        started = perf_counter()
        ...
        if corpus is None:
            ...
            try:
                answer_engine, scope, vector_store, answer_provider = build_answer_stack(
                    corpus,
                    repository_id=repo_id,
                    scan_id=scan_id,
                    settings=acceptance_settings,
                )
                rows = ask_predefined_questions(answer_engine, scope)
        ...
        mcp_label = "not_run"
        try:
            health_payload = mcp_health_check(
                acceptance_settings,
