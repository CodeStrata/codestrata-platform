...
from aimf.domain.evidence.dependency.models import DependencyDeclarationEvidence
...
class UnboundedRequirementRule:
    """Flags Python registry-style declarations with no version constraint."""

    @property
    def metadata(self) -> RuleMetadata:
        return make_metadata(
            rule_id=RULE_UNBOUNDED_REQUIREMENT,
            title="Unbounded Python requirement",
            description=(
                "Detects supported Python registry-style dependency declarations "
                "that contain no version constraint. Local, editable, URL, and VCS "
    ...
    def evaluate(self, context: RuleExecutionContext) -> SharedRuleEvaluationResult:
        ...
        matches = []
        for item in evidence.declarations:
            if item.ecosystem is not DependencyEcosystem.PYTHON:
                continue
            if is_local_or_direct_reference(item):
                continue
            ...
            matches.append(
                match(
                    ...
                    title="Unbounded Python requirement",
                    summary=(
                        f"Python package '{item.normalized_identity}'{extras_note} "
                    ...
                    evidence=(
                        evidence_declaration(
                            item=item,
                            message=(
                                "Declaration has no version specifier; the "
                                "selected version is unconstrained by this "
                                "manifest entry."
                            ),
                        ),
                    ),
