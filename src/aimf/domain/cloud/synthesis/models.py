"""Cloud synthesis domain models (Phase 4.7.5).

Themes, conclusions, and recommendations derived from the assessment inventory.
No scores, grades, risk indices, modernization guidance, or AI narrative.
"""
...
class CloudTheme(BaseModel):
    """Inventory-derived Cloud theme (no synthetic score)."""
    ...
    title: str
    description: str
    scope: CloudThemeScope = CloudThemeScope.REPOSITORY
...
class CloudConclusion(BaseModel):
    """Deterministic Cloud conclusion with bounded template text."""
    ...
    assessment_dimensions: tuple[str, ...] = ("cloud",)
    business_impact: str = "unknown"
    confidence: str = "high"
    provenance: str = "cloud_synthesis"
...
class CloudSynthesisResult(BaseModel):
    """Complete synthesis payload attached to the assessment section."""

    model_config = ConfigDict(frozen=True, extra="forbid")

    status: CloudSynthesisStatus = CloudSynthesisStatus.SUCCEEDED
