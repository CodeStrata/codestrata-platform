"""AI Readiness report presentation models (Phase 4.8.6)."""
...
AI_READINESS_REPORT_SECTION_ID = "report.ai_readiness"
AI_READINESS_REPORT_SECTION_VERSION = "1.0.0"
...
CAPABILITY_FAMILY_DISPLAY_LIMIT = 12


class AiReadinessReportCountBucket(BaseModel):
...
class AiReadinessReportInventorySummary(BaseModel):
    ...
    by_rule: tuple[AiReadinessReportCountBucket, ...] = ()
    by_severity: tuple[AiReadinessReportCountBucket, ...] = ()
    by_confidence: tuple[AiReadinessReportCountBucket, ...] = ()
...
class AiReadinessReportCapabilityFamilyEntry(BaseModel):
    """One capability family coverage row for presentation."""
...
class AiReadinessReportCapabilityFamilySummary(BaseModel):
    """Capability family inventory projection (Finding-derived)."""

    model_config = ConfigDict(frozen=True, extra="forbid")

    families_observed: int = Field(default=0, ge=0)
    families_total: int = Field(default=0, ge=0)
    entries: tuple[AiReadinessReportCapabilityFamilyEntry, ...] = ()
    note: str = (
        "Capability families summarize Finding-derived AI readiness signal "
        "groups only. They do not establish enablement suitability or produce "
        "scores."
    )
...
class AiReadinessReportRuleEntryView(BaseModel):
    model_config = ConfigDict(frozen=True, extra="forbid")
...
class AiReadinessReportExecutionSummary(BaseModel):
    model_config = ConfigDict(frozen=True, extra="forbid")
    ...
    entries: tuple[AiReadinessReportRuleEntryView, ...] = ()
    note: str = (
        "Rule execution summarizes registered AI Readiness Hygiene rules only. "
        "Zero findings are a bounded inventory outcome and do not establish "
        "enablement suitability."
    )
...
class AiReadinessReportCoverageAreaView(BaseModel):
    model_config = ConfigDict(frozen=True, extra="forbid")
...
class AiReadinessReportCoverageSummary(BaseModel):
    model_config = ConfigDict(frozen=True, extra="forbid")

    evidence_pipeline: str = ""
    evidence_status: str = ""
    areas: tuple[AiReadinessReportCoverageAreaView, ...] = ()
    note: str = (
        "Coverage describes AI Readiness assessment capability areas only. "
        "Agent execution, RAG suitability, and readiness scores are not measured."
    )
...
class AiReadinessReportThemeView(BaseModel):
    model_config = ConfigDict(frozen=True, extra="forbid")
...
class AiReadinessReportConclusionView(BaseModel):
    model_config = ConfigDict(frozen=True, extra="forbid")
...
class AiReadinessReportRecommendationView(BaseModel):
    model_config = ConfigDict(frozen=True, extra="forbid")
...
class AiReadinessReportRecommendationGroup(BaseModel):
    model_config = ConfigDict(frozen=True, extra="forbid")

    group: str
    recommendations: tuple[AiReadinessReportRecommendationView, ...] = ()
...
class AiReadinessReportDiagnosticView(BaseModel):
    model_config = ConfigDict(frozen=True, extra="forbid")
...
class AiReadinessReportLimitationView(BaseModel):
    model_config = ConfigDict(frozen=True, extra="forbid")
...
class AiReadinessReportTraceEdgeView(BaseModel):
    model_config = ConfigDict(frozen=True, extra="forbid")
...
class AiReadinessReportTraceabilityView(BaseModel):
    ...
    summary: str = ""
    finding_ids: tuple[str, ...] = ()
    sample_edges: tuple[AiReadinessReportTraceEdgeView, ...] = ()
...
class AiReadinessReportSection(BaseModel):
    """Presentation-focused AI Readiness Intelligence report section."""

    model_config = ConfigDict(frozen=True, extra="forbid")

    section_id: str = AI_READINESS_REPORT_SECTION_ID
    section_version: str = AI_READINESS_REPORT_SECTION_VERSION
    schema_name: str = "report.ai_readiness"
    title: str = "AI Readiness Intelligence"
    status: str
    status_label: str
    status_summary: str
    ...
    coverage_summary: AiReadinessReportCoverageSummary = Field(
        default_factory=AiReadinessReportCoverageSummary
    )
    execution_summary: AiReadinessReportExecutionSummary = Field(
        default_factory=AiReadinessReportExecutionSummary
    )
    inventory_summary: AiReadinessReportInventorySummary = Field(
        default_factory=AiReadinessReportInventorySummary
    )
    capability_family_summary: AiReadinessReportCapabilityFamilySummary = Field(
        default_factory=AiReadinessReportCapabilityFamilySummary
    )
    themes: tuple[AiReadinessReportThemeView, ...] = ()
    themes_displayed: int = Field(default=0, ge=0)
    themes_total: int = Field(default=0, ge=0)
    conclusions: tuple[AiReadinessReportConclusionView, ...] = ()
    conclusions_displayed: int = Field(default=0, ge=0)
    conclusions_total: int = Field(default=0, ge=0)
    recommendations: tuple[AiReadinessReportRecommendationView, ...] = ()
    recommendation_groups: tuple[AiReadinessReportRecommendationGroup, ...] = ()
    recommendations_displayed: int = Field(default=0, ge=0)
    recommendations_total: int = Field(default=0, ge=0)
    diagnostics: tuple[AiReadinessReportDiagnosticView, ...] = ()
    diagnostics_displayed: int = Field(default=0, ge=0)
    diagnostics_total: int = Field(default=0, ge=0)
    limitations: tuple[AiReadinessReportLimitationView, ...] = ()
    limitations_displayed: int = Field(default=0, ge=0)
    limitations_total: int = Field(default=0, ge=0)
    traceability: AiReadinessReportTraceabilityView = Field(
        default_factory=AiReadinessReportTraceabilityView
    )
    ...
    @classmethod
    def normalize_required(cls, value: object) -> str:
        return require_nonblank(str(value), label="ai readiness report field")
