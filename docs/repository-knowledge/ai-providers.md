...
* `pip install 'ai-modernization-factory[bedrock]'`
* `pip install 'ai-modernization-factory[openai]'`

## Registry and factories

`AIProviderRegistry` (`aimf.ai.providers.registry`) registers:
...
`AimfSettings` is supplied (MCP and CLI do this).
...
```bash
aimf ai providers
aimf ai config --config aimf.toml
aimf ai health --config aimf.toml
aimf ai health --live --config aimf.toml
```
