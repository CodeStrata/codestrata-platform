---
title: Public Contracts
description: Stable public contracts for CodeStrata Engine reports, CLI, and integrations.
---

# Public Contracts

Public contracts define what Community clients may rely on:

- CLI command surfaces for assessment journeys
- Report and JSON artifact shapes used by IDE extensions
- Compatibility expectations between Engine and extension clients

Authoritative technical definitions ship with **CodeStrata Engine**. This portal
explains how contracts fit the Community developer journey; Engine `docs/` retains
schema and implementation detail.
