...
| ------------- | ------ |
| 4.6.1 Domain Foundation | Complete (`testing-assessment` 1.0.0) |
...
| 4.6.4+ Inventory / Synthesis / Report | Not started |
...
Still out of scope: inventories, deterministic synthesis, report projection,
CLI/MCP, test execution, and runtime coverage measurement.
...
| Future report | `report.testing` (not in this phase) |
...
No synthesis option or `report.sections.testing` yet.
...
| on | on | usable | `succeeded` with hygiene findings (possibly zero) |
...
Inventories, deterministic synthesis, presentation-only report adapter,
CLI/MCP, TEST-004, test execution, runtime coverage.
